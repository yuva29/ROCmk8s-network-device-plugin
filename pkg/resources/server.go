...
import (
	...
	"github.com/k8snetworkplumbingwg/sriov-network-device-plugin/pkg/deviceconfig"
	"github.com/k8snetworkplumbingwg/sriov-network-device-plugin/pkg/exporter"
...
type resourceServer struct {
	...
	DevConfigHandler        *deviceconfig.DevConfigHandler
	exporterHealthCheckChan <-chan time.Time
	metricsClient           *exporter.MetricsClient
...
func NewResourceServer(prefix, suffix string, pluginWatch, useCdi bool, rp types.ResourcePool) types.ResourceServer {
	...
	rs := &resourceServer{
		...
		DevConfigHandler:   deviceconfig.NewDevConfigHandler(),
	}

	if rp.EnableExporterHealthCheck() {
		ticker := time.NewTicker(healthCheckInterval) // health check every 30 seconds
		rs.exporterHealthCheckChan = ticker.C
		rs.metricsClient = exporter.NewMetricsClient()
	}

	if rp.DisableDeviceConfig() {
		glog.Info("disabling configuration of devices before Allocate\n")
	} else {
		glog.Info("enabling configuration of devices before Allocate\n")
		deviceConfigClient := deviceconfig.NewDevConfigClient()
		rs.DevConfigHandler.RegisterDevClient(deviceConfigClient)
	}
...
func (rs *resourceServer) Allocate(ctx context.Context, rqt *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse, error) {
	...
	for _, container := range rqt.ContainerRequests {
		...
		err = rs.DevConfigHandler.SetupDeviceHw(container.DevicesIDs)
		if err != nil {
			glog.Errorf("setup device failed for IDs %v: %v", container.DevicesIDs, err)
			return nil, err
		}
...
func (rs *resourceServer) ListAndWatch(empty *pluginapi.Empty, stream pluginapi.DevicePlugin_ListAndWatchServer) error {
	...
	for {
		...
		case <-rs.exporterHealthCheckChan:
			rs.metricsClient.UpdateDevicesHealth(devs, pluginapi.Healthy)
			resp.Devices = devs
			if err := stream.Send(resp); err != nil {
				glog.Errorf("%s: error: cannot update device states: %v\n", methodID, err)
				return err
...
func (rs *resourceServer) Start() error {
	...
	glog.Infof("initializing %s device setup clients \n", resourceName)
	err = rs.DevConfigHandler.InitDeviceClients()
	if err != nil {
		glog.Errorf("error initializing %s device setup clients \n", resourceName)
		return err
	}
...
func (rs *resourceServer) Stop() error {
	...
	if rs.metricsClient != nil {
		rs.metricsClient.Close()
		rs.metricsClient = nil
	}
